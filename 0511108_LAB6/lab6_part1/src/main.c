		...
		for(int a=0;a<button_press_cycle_per_second;a++){
			...
			if(pos_cnt>debounce_threshold){
				if(last_button_state==0){
					...
					x=1+x;
					//pressed 1 time, change freq to 0.1s bright led
					if(x==1){
					SystemClock_Config_Interrupt(10, 1000000);
					//x=1+x;
					}
					//pressed 2nd times, change freq to 0.3s bright led
					else if(x==2){
					SystemClock_Config_Interrupt(10, 3000000);
					//x=1+x;
					}
					//pressed third times, turn back to the initial freq and reset to 0.5s bright led
					else {
					x=3-x;
					SystemClock_Config_Interrupt(10, 5000000);
					}
